...
#include <token.h> \\ for asmtok definition
...
//    enum exprtype   kind;	    /* Type of expression */
    ...
    unsigned char   Ofssize;	    /* 16,32,64 bit if MT_NEAR, MT_FAR */
    union {
	uint_8	    flags1;
	struct {
	    unsigned char indirect : 1;	 /* indirect addressing used */
	    unsigned char explicit : 1;	 /* Whether expression type explicitly given (to be removed!) */
	    unsigned char is_abs   : 1;	 /* external ABS */
	    unsigned char is_type  : 1;	 /* constant is a type */
	    unsigned char is_opattr: 1;	 /* current operator is OPATTR */
	    unsigned char negative : 1;	 /* for EXPR_FLOAT only */
	    unsigned char assumecheck: 1;/* v2.07: for ASSUMEd std registers */
	    unsigned char is_dot: 1;	    /* v2.10: see regression test dotop5.asm */
	};
    };
